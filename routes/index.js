...
router.get('/cost-by-day', function(req, res) {
  res.render('cost-by-day');
});
...
router.get('/data/cost-by-day', function(req, res) {
  ...
  MongoClient.connect(mongodbUrl, function(err, db) {
    ...
    collection.aggregate(aggQuery, function(err, result) {
      ...
      earliest = moment(earliest).add(-1, 'day');
      latest = moment(latest);

      var dataSeries = []

      var cost = 0;
      for(var now = earliest.clone(); now <= latest; now.add(1, 'day')) {

        var newDateString = 'new Date('+now.year()+', '+(now.month())+', '+now.date()+', '+now.hour()+')';
        var count = (dataMap[now.format("YYYY-MM-DD")] || 0)/100;
